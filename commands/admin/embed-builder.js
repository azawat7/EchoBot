...
module.exports.help = {
  name: "embed-builder",
  aliases: ["embed-constructor", "embed"],
  category: "admin",
  expectedArgs: null,
  minArgs: 0,
  maxArgs: 0,
...
module.exports.run = async (client, message, args, language, settings) => {
  ...
  const embedToBuild = new MessageEmbed().setAuthor(
  ...
  let embedEditing = async (button) => {
    ...
    if (id.startsWith(`builder`)) {
      ...
      button.message.edit(
        `Waiting for input... ${
          builderId.includes("field") ? "(field name)" : ""
        }`,
        {
      ...
      if (builderId == "color") {
        if (!/^#[0-9A-F]{6}$/i.test(finalInput.content))
          embedToBuild.setColor("RANDOM");
      ...
      if (builderId == "addfield") {
        button.message.edit(
          `Waiting for input... ${
            builderId == "addfield" ? "(field value)" : ""
          }`,
          {
            components: new MessageActionRow().addComponent(
              new MessageButton()
      ...
      if (builderId == "removefield") {
        let toRemove = embedToBuild.fields.find(
          (f) => f.name == finalInput.content
        );
        let newFields = embedToBuild.fields.find(
          (f) => f.name != finalInput.content
        );
    ...
    if (id == `save`) {
      let messageToDelete = await button.channel.messages.fetch(
        button.message.id
      );
      messageToDelete.delete();

      button.channel.send({ embed: embedToBuild, components: [] });
      await client.removeListener("clickButton", buttonEvent);
    }
